...
-- Update existing resources to have a placeholder user_id (will be null for existing resources)
...
CREATE POLICY "Users can update download count" 
ON public.resources 
FOR UPDATE 
USING (true)
WITH CHECK (
  -- Only allow updating the downloads column
  OLD.id = NEW.id AND
  ...
  OLD.uploaded_by = NEW.uploaded_by AND
  OLD.file_size = NEW.file_size AND
